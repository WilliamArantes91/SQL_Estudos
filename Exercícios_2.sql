...
Select distinct top(10) * from DimProduct order by UnitPrice desc, Weight desc, AvailableForSaleDate asc
...
select ProductName as 'Produto',Weight as 'Peso' from DimProduct where Weight >= 100 order by Weight desc
...
select StoreName as 'Lojas', OpenDate as 'Data de abertura', EmployeeCount as 'Qtd.Funcionários' from DimStore where CloseDate is null
...
select * from DimProduct where BrandName = 'Litware' and ProductName like('%Home Theater%') and AvailableForSaleDate = '20090315'
...
select * from DimStore where OpenDate is not null
...
select StoreName from DimStore where EmployeeCount >= 51
...
select ProductKey as 'ID',ProductName as 'Produto', UnitPrice as 'Preço' from DimProduct where ProductName like '%LCD%'
...
select * from DimProduct where BrandName in ('Contoso','Litware','Fabrikam') and ColorName in ('Green', 'Orange', 'Black', 'Silver', 'Pink')
...
select * from DimProduct where BrandName = 'Contoso' and ColorName = 'Silver' and Weight between 10 and 30 order by UnitPrice desc